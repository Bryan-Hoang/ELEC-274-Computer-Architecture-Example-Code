...
# Rudimentary interrupt handler
	...
	# This code will be executed when interrupt, exception or TRAP instruction
	# occurs.  Code is executed in Supervisor mode (if enabled).
	# Code uses general purpose registers r18,r19 - must be saved and restored
	subi	sp, sp, 8		# room on stack for two words
	...
	# note that rdctl instruction - to read control register - can only be
...
.endif
	...
	andi	r19, et, 0x100	# Check bit 8
	beq		r19, r0, NOTIRQ8
	call	EXT_IRQ8		# Call code to handle JTAG port
...
EXT_IRQ0:	# handle IRQ0 interrupt request
	ret 
EXT_IRQ1:	# handle IRQ1 interrupt request
	ret 
	
EXT_IRQ8:	# handle interrupts from JTAG device
	# Two reasons device may interrupt - empty transmit buffer or new character
	# received.
